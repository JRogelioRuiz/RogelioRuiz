...
bool Log::operator<(Log log) {
    ...
    if (ipValue != log.ipValue)
        return ipValue < log.ipValue;
    else if (yearValue != log.yearValue)
...
bool Log::operator==(Log log) {
    if (ipValue == log.ipValue && yearValue == log.yearValue && monthValue == log.monthValue && dayValue == log.dayValue && timeValue == log.timeValue)
        return true;
    else
        return false; 
}
